/* 
 * PruebaInterpoladoresPolinomicos.java
 * 
	...
	static class Resultados extends JPanel implements MouseListener{
		...
		private void rellenar(){
			for (int i = 0; i < N_PUNTOS; i++){
				keys[i] = i== 0 ? 0 : keys[i-1]+(float)Math.max(0.1,Math.random());
//				keys[i] = 1.0f*i/(N_PUNTOS-1);
			...
			for (int i = 0; i < N_PUNTOS; i++)
				keys[i] /= keys[N_PUNTOS-1];
			interpolador = GettersFactory.Float.create(keys, values, MetodoDeInterpolacion.Predefinido.FINITE_DIFFERENCE_SPLINE);
		}
