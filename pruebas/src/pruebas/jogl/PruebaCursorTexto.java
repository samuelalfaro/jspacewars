...
import pruebas.jogl.PruebaCursor.CursorListener;

import com.jogamp.opengl.util.Animator;

public class PruebaCursorTexto{
	
	private static class TextRenderer implements GLEventListener{
		
		private static final int VIRTUAL_AREA_WIDTH  = 1024;
		private static final int VIRTUAL_AREA_HEIGHT =  768;

		private static final String fontDef = "resources/texturas/fonts/arbeka.xml";
		private static final String font1Texture = "resources/texturas/fonts/arbeka.png";
		private static final String font2Texture = "resources/texturas/fonts/arbeka-blur.png";
		...
		public void reshape( GLAutoDrawable glDrawable, int x, int y, int width, int height ){
			if( width != 0 && height != 0 ){
				viewport.x      = ( VIRTUAL_AREA_HEIGHT * width - VIRTUAL_AREA_WIDTH * height ) 
				                          / ( 2 * VIRTUAL_AREA_HEIGHT );
				viewport.y      = 0;
				viewport.width  = VIRTUAL_AREA_WIDTH * height / VIRTUAL_AREA_HEIGHT;
	...
	private static class CursorRenderer implements GLEventListener{
		...
		CursorRenderer( Matrix4f tCursor ){
		...
		public void display( GLAutoDrawable glDrawable ){
			...
			gl.glMatrixMode( GLMatrixFunc.GL_PROJECTION );
			gl.glLoadIdentity();
			gl.glOrtho( viewport.x, viewport.width, viewport.height, viewport.y, -1, 1 );
	...
	public static void main( String[] args ){
		...
		JFrame frame = new JFrame( "Prueba Texto y Cursor" );
		...
		canvas.addGLEventListener( new TextRenderer() );
		...
		canvas.addGLEventListener( new CursorRenderer( tCursor ) );
