...
public class PruebaDots{
	...
	static float sBoxPulse( float value, float upSlopeStart, float upSlopeEnd, float downSlopeStart, float downSlopeEnd ){
		return sBoxStep( value, upSlopeStart, upSlopeEnd ) - sBoxStep( value, downSlopeStart, downSlopeEnd );
	}

	static float sSmoothPulse( float value, float upSlopeStart, float upSlopeEnd, float downSlopeStart,
			float downSlopeEnd ){
	...
	private static class Dots{
		...
		public void init( Image img, int startx, int starty, int endtopleftx, int endtoplefty, float imgscale,
	...
	private static class Renderer implements GLEventListener{
		...
		public void init( GLAutoDrawable drawable ){
			...
			fondo.setTextura( new Textura( gl, Textura.Format.RGB,
					Imagen.cargarToBufferedImage( "resources/texturas/cielo512.jpg" ), true ) );
		...
		private void drawBackground( GL2 gl, int imgW, int imgH ){
		 ...
		 */
		private void drawDots( GL2 gl, long time, long startTime, long timeSpan, Dots credit, Dots name ){
		...
		private void drawFade( GL2 gl, long time, int startTime, int stopTime, boolean toBlack ){
