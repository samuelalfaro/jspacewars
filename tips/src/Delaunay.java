...
public class Delaunay extends Applet {
	...
	public void init() {
		...
		p.add(cb_vor = new Checkbox("Voronoi Diagram"));
	...
	public boolean action(Event e, Object o) {
		...
		if( e.target == cb_pt ){
			dp.showpt = cb_pt.getState();
			dp.repaint();
			return true;
		}
		if( e.target == cb_tri ){
			dp.showtri = cb_tri.getState();
			dp.repaint();
			return true;
		}
		if( e.target == cb_vor ){
			dp.showvor = cb_vor.getState();
			dp.repaint();
			return true;
		}
		if( e.target == cb_cc ){
			dp.showcir = cb_cc.getState();
			dp.repaint();
			return true;
		}
		return false;
...
class DrawPad extends Panel {
	...
	public void paint(Graphics g) {
		if( showcir == true )
			dt.DrawCircles(g, Color.white);
		if( showtri == true )
			dt.DrawTriangles(g, Color.yellow);
		if( showvor == true )
			dt.DrawVoronoiDiagram(g, Color.blue);
		if( showpt == true )
			dt.DrawPoints(g, Color.black);
	}
...
class Triangle extends Object {
	...
	public Triangle(Vector<Edge> edges, Edge e1, Edge e2, Edge e3) {
		Update(e1, e2, e3);
		edges.addElement(e1);
		edges.addElement(e2);
		edges.addElement(e3);
	...
	void RemoveEdges(Vector<Edge> edges) {
	...
	void FindCircle() {
		double x1 = anEdge.P1().x;
		double y1 = anEdge.P1().y;
		double x2 = anEdge.P2().x;
		double y2 = anEdge.P2().y;
		double x3 = anEdge.NextE().P2().x;
		double y3 = anEdge.NextE().P2().y;
	...
	public void DrawCircles(Graphics g) {
		int x0 = (int) (c_cx - c_r);
		int y0 = (int) (c_cy - c_r);
		int diametro = (int) (2.0 * c_r);
		// g.drawOval((int) (c_cx - c_r), (int) (c_cy - c_r), (int) (2.0 * c_r),
		// (int) (2.0 * c_r));
...
class DelaunayT {
	Vector<Node> nodes; // nodes set
	Vector<Edge> edges; // edges set
	Vector<Triangle> tris; // triangles set
	...
	public void Insert(int px, int py, Object data) {
		int eid;
		Node nd = new Node(px, py, data);
		nodes.addElement(nd);
		if( nodes.size() < 3 )
			return;
		if( nodes.size() == 3 ) // create the first triangle
		{
			Node p1 = nodes.elementAt(0);
			Node p2 = nodes.elementAt(1);
			Node p3 = nodes.elementAt(2);
			...
			{
				p1 = nodes.elementAt(1);
				p2 = nodes.elementAt(0);
				e1.Update(p1, p2);
			}
	...
	public void Delete(int px, int py) {
		...
		int nodetype = 0;
		int idegree = -1;
		if( index.length < edges.size() )
			index = new Edge[edges.size() + 100];
		while( nodetype == 0 ){
			...
			if( e == null )
				nodetype = 2; // nd on convex hull
			if( e == start )
				nodetype = 1; // inner node
		}
	...
	void ExpandTri(Edge e, Node nd, int type) {
		...
		}else // nd is on the edge e
		{
			...
			if( e4 == null || e4.Tri() == null ) // one triangle involved
			{
				...
				hullStart = e02;
				tris.removeElement(e1.Tri()); // remove oldtriangle // add two
				// new triangles
			...
			}else // two triangle involved
			{
				...
				e5.AsIndex(); // because e, e4 removed, reset edge index of node
				// p1 and p2
				e2.AsIndex();
				tris.addElement(new Triangle(edges, e2, e30, e20.MakeSymm()));
				tris.addElement(new Triangle(edges, e3, e10, e30.MakeSymm()));
	...
	public void DrawPoints(Graphics g, Color color) {
		g.setColor(color);
		for( Node node: nodes )
			g.drawRect(node.x - 1, node.y - 1, 2, 2);
	}

	public void DrawTriangles(Graphics g, Color color) {
		g.setColor(color);
		if( nodes.size() == 1 )
			g.drawRect(nodes.elementAt(0).x, nodes.elementAt(0).y, 1, 1);
		if( nodes.size() == 2 )
			g.drawLine(nodes.elementAt(0).x, nodes.elementAt(0).y, nodes.elementAt(1).x, nodes.elementAt(1).y);
