...
public class PruebaMovimiento{
	...
	static class Nave extends Elemento{
		 ...
		 * @param forma
		 */
		public Nave( int tipo, Polygon forma, int[][] limitesDeNiveles, float[] velocidadesDisponibles,
				Canion[][][] canionesDisponibles, List<Disparo> disparos ){
			super( tipo, forma );
	...
	static abstract class Canion implements Cloneable{
		...
		protected int grado;
		/**
		 * Referencia a los distintos disparos que puede lanzar un cañon dependiendo de su grado.
		 */
	...
	static class Motor{
		...
		synchronized void actua( int keyState, long milis ){
			while( !pintado )
				Thread.yield();
	...
	static public void main( String args[] ){
		...
		int limitesDeNiveles[][] = new int[][] { { 0, 4, 3, 2, 0 }, { 2, 2, 2, 1, 1 }, { 4, 4, 4, 2, 2 },
				{ 5, 5, 5, 3, 3 } };

		float velocidadesDisponibles[] = new float[] { 200.0f, 600.0f, 300.0f, 350.0f, 400.0f };
		...
		Canion[][][] canionesDisponibles = new Canion[][][] {
				// Cañones primarios
				{ { c11 }, { c12, c13 }, { c11, c12, c13 }, { c12, c13, c14, c15 }, { c11, c12, c13, c14, c15 }, },
				// Cañones secundarios
				{ {}, { c21, c22 }, { c21, c22, c23, c24, c25, c26 }, }, };

		Cache<Elemento> cache = new Cache<Elemento>( 500 );
		Canion.setCache( cache );
		LinkedList<Disparo> disparos = new LinkedList<Disparo>();

		Nave nave = new Nave( 101, formas.removeFirst(), limitesDeNiveles, velocidadesDisponibles, canionesDisponibles,
				disparos );
		nave.inciarNave();
		...
		double values[][] = new double[][] { { 0.0, 0.0 }, { -10.0, 0.0 }, { -25.0, 15.0 }, { -25.0, 25.0 },
				{ -25.0, 35.0 }, { -10.0, 50.0 }, { 0.0, 50.0 }, { 25.0, 50.0 }, { 75.0, -25.0 }, { 100.0, -25.0 },
				{ 125.0, -25.0 }, { 175.0, 25.0 }, { 200.0, 25.0 }, { 225.0, 25.0 }, { 275.0, -25.0 },
				{ 300.0, -25.0 }, { 325.0, -25.0 }, { 375.0, 25.0 }, { 400.0, 25.0 }, { 425.0, 25.0 },
				{ 475.0, -25.0 }, { 500.0, -25.0 }, { 525.0, -25.0 }, { 575.0, 25.0 }, { 600.0, 25.0 },
				{ 625.0, 25.0 }, { 675.0, -25.0 }, { 700.0, -25.0 } };

		Trayectoria.Double<double[]> trayectoria = (Trayectoria.Double<double[]>)GettersFactory.Double.create(
				Keys.PROPORCIONALES, 2.5, 0.0, values, MetodoDeInterpolacion.Predefinido.BEZIER );
