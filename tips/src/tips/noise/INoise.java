...
public final class INoise {
	...
	static int fade(int t) {
		int t0 = fade[t >> 8];
		int t1 = fade[Math.min(255, (t >> 8) + 1)];
	...
	static public int noise(int x, int y, int z) {
		...
		int x0 = x & 0xFFFF, x1 = x0 - 0x10000;
		int y0 = y & 0xFFFF, y1 = y0 - 0x10000;
		int z0 = z & 0xFFFF, z1 = z0 - 0x10000;
		...
		return lerp(
					...
					lerp(
						v, 
						lerp(u, grad(p[AA], x0, y0, z0), grad(p[BA], x1, y0, z0)),
						lerp(u, grad(p[AB], x0, y1, z0), grad(p[BB], x1, y1, z0))
					),
					lerp(
						v,
						lerp(u, grad(p[AA + 1], x0, y0, z1), grad(p[BA + 1], x1, y0, z1)),
						lerp(u, grad(p[AB + 1], x0, y1, z1), grad(p[BB + 1], x1, y1, z1))
					)
				);
	}
}
