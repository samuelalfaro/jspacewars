...
public class CMYK_OOo_ColorTableGenerator {
	
	public static class CMYKColorSpace extends ColorSpace {
	...
	static void printRGB(float rgb[]){
		System.out.format("<draw:color draw:name=\"%1$02X%2$02X%3$02X\" draw:color=\"#%1$02x%2$02x%3$02x\"/>\n",(int)(rgb[0]*255),(int)(rgb[1]*255),(int)(rgb[2]*255));
	}
	...
	public static void main(String[] args) {
		...
		for(int c = 10; c > 0; c--){
			float cf = c/10.0f;
			float cf2 = c/20.0f;
			printRGB(colorSpace.toRGB(new float[]{  0,  0,  0, cf}));
			printRGB(colorSpace.toRGB(new float[]{  0,cf2, cf,  0}));
			printRGB(colorSpace.toRGB(new float[]{  0, cf,cf2,  0}));
			printRGB(colorSpace.toRGB(new float[]{  0, cf,  0,  0}));
			printRGB(colorSpace.toRGB(new float[]{cf2, cf,  0,  0}));
			printRGB(colorSpace.toRGB(new float[]{ cf,cf2,  0,  0}));
			printRGB(colorSpace.toRGB(new float[]{ cf,  0,  0,  0}));
			printRGB(colorSpace.toRGB(new float[]{ cf,  0,cf2,  0}));
			printRGB(colorSpace.toRGB(new float[]{cf2,  0, cf,  0}));
			printRGB(colorSpace.toRGB(new float[]{  0,  0, cf,  0}));
		}
		printRGB(new float[]{1,1,1});
	}
