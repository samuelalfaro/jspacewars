...
public class Puff extends Coulds {
	...
	/* (non-Javadoc)
	 * @see PerlinNoise#setOctaves(int)
	 */
	@Override
	...
	public double calculate(int x, int y) {
		...
		for( int i = 0; i < this.octaves; i++ ){
			...
			double partial = (len > radius) ? 
					0 :
					(radius - len )/radius *
//					( Noise3.noise( (x + 128) * freq, (y +128) * freq, this.z) ) *
					( SimplexNoise.noise( x * freq, y * freq, this.z) ) *
//					(INoise.noise( (int)(x * freq * this.width * this.width), (int)(y * freq * this.width * this.width), (int)( this.z * this.width * this.width )) )*
					ampl;
