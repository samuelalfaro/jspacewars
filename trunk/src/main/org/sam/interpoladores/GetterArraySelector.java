 ...
 * <p>
 * Se considera un <i>GetterArraySelector</i> a un tipo de <i>Getter</i> que contiene los valores distribuidos
 * uniformente, por tanto no es necesario almacenar las claves. En esta implemetación se almacena la escala y traslación
 * que se aplica a la clave para obtener el índice del vector dónde se encuentra el valor que será devuelto.
 * </p>
 * <p>
 * Esta implementación es usada por {@code GettersFactory} cuando no pasan claves y el {@code MetodoDeInterpolacion}
 * seleccionado es {@linkplain MetodoDeInterpolacion.Predefinido#ESCALON}.
 * </p>
 * <p>
 * <u>Nota:</u> Los valores no se interpolan realmente, pasando bruscamente de un valor a otro, lo que da esta forma
 * característica de escalón.
 * </p>
 */
final class GetterArraySelector{
	...
	/**
	 * Método que devuelve el elemento de un determinado vector, sin lanzar excepciones.<br/>
	 * Si:<ul>
	 * <li>{@code index < 0}: devuelve el elemento almacenado en la posición 0.</li>
	 * <li>{@code index >= tamaño del vector}: devuelve el elemento almacenado en la última posición.</li>
	 * <li>resto de casos: devuelve elemento alamacenado en la posición {@code index}.</li>
	 * </ul>
	 * @param <T> Tipo de datos del vector.
	 * @param index índice del elemento solicitado.
	 * @param array vector del que se obtiene el valor.
	 * @return el elemento solicitado.
	 */
	static <T>T valueAt( int index, T[] array ){
	...
	final static class Double <T> implements Getter.Double<T>{
		...
		/**
		 * Constructor que crea un {@code GetterArraySelector} con precisión {@code double}, con una escala = 1 y una
		 * traslación = 0.
		 * 
	...
	final static class Float <T> implements Getter.Float<T>{
		...
		/**
		 * Constructor que crea un {@code GetterArraySelector} con precisión {@code float}, con una escala = 1 y una
		 * traslación = 0.
		 * 
		 * @param values valores que serán devueltos.
		 */
	...
	final static class Integer <T> implements Getter.Integer<T>{
		...
		/**
		 * Constructor que crea un {@code GetterArraySelector} con precisión {@code int}, con una escala = 1 y una
		 * traslación = 0.
		 * 
		 * @param values valores que serán devueltos.
		 */
