...
public class XStreamConventers {

	private XStreamConventers(){}
	...
	private static void write(Object values, HierarchicalStreamWriter writer, MarshallingContext context){
	...
	private static class GetterDoubleConverter implements Converter {
		...
		public void marshal(Object value, HierarchicalStreamWriter writer, MarshallingContext context) {
			...
			write(params.values, writer, context);
	...
	private static class GetterIntegerConverter implements Converter {

		private static class IntegerParams{
			private int keys[];
			private int keysGen;
			private int keysScale;
			private int keysTranslation;
			private Object values;
			private Introductor.Integer<?> introductor;
			private Extractor<?> extractor;
			private MetodoDeInterpolacion metodoDeInterpolacion;
			Object mdiParams[];
		}
		...
		@SuppressWarnings(S.unchecked)
		private static <T> Getter.Integer<?> create(IntegerParams params){
			if(params.keys != null){
				if(params.values instanceof Integer[])
					return GettersFactory.Integer.create(params.keys, (Integer[])params.values, params.metodoDeInterpolacion, params.mdiParams );
				if(params.values instanceof int[])
					return GettersFactory.Integer.create(params.keys, (int[])params.values, params.metodoDeInterpolacion, params.mdiParams );
				if(params.values instanceof int[][])
					return GettersFactory.Integer.create(params.keys, (int[][])params.values, params.metodoDeInterpolacion, params.mdiParams );
				return GettersFactory.Integer.create(params.keys, null, (Introductor.Integer<T>) params.introductor, (T[])params.values, (Extractor<? super T>)params.extractor, params.metodoDeInterpolacion, params.mdiParams );
			}
			if(params.values instanceof Integer[])
				return GettersFactory.Integer.create(params.keysGen, params.keysScale, params.keysTranslation, (Integer[])params.values, params.metodoDeInterpolacion, params.mdiParams );
			if(params.values instanceof int[])
				return GettersFactory.Integer.create(params.keysGen, params.keysScale, params.keysTranslation, (int[])params.values, params.metodoDeInterpolacion, params.mdiParams );
			if(params.values instanceof int[][])
				return GettersFactory.Integer.create(params.keysGen, params.keysScale, params.keysTranslation, (int[][])params.values, params.metodoDeInterpolacion, params.mdiParams );
			return GettersFactory.Integer.create(params.keysGen, params.keysScale, params.keysTranslation, null, (Introductor.Integer<T>) params.introductor, (T[])params.values, (Extractor<? super T>)params.extractor, params.metodoDeInterpolacion, params.mdiParams );
		}
		
		@SuppressWarnings(S.unchecked)
		public boolean canConvert(Class clazz) {
			return Getter.Integer.class.isAssignableFrom(clazz)
					|| IntegerParams.class == clazz;
		}

		public void marshal(Object value, HierarchicalStreamWriter writer, MarshallingContext context) {
			IntegerParams params = (IntegerParams)value;
			...
			if (params.keys != null) {
				...
				if(params.keysGen == Keys.PROPORCIONALES)
					...
					writer.addAttribute(S.keyGen, S.HOMOGENEAS);
				writer.addAttribute(S.scale, ((Integer) params.keysScale).toString());
				writer.addAttribute(S.translation, ((Integer) params.keysTranslation).toString());
			}
			writer.endNode();
		...
		public Object unmarshal(HierarchicalStreamReader reader, UnmarshallingContext context) {
			try{
				IntegerParams params = new IntegerParams();
				String att;
				...
				while (reader.hasMoreChildren()) {
					...
					if (reader.getNodeName().equals(S.Keys))
						if (reader.getAttribute(S.length) != null)
							params.keys = (int[]) context.convertAnother(null, int[].class);
						else {
							att = reader.getAttribute(S.keyGen);
							if( att != null && att.equals(S.PROPORCIONALES))
								params.keysGen = Keys.PROPORCIONALES;
								...
								params.keysGen = Keys.HOMOGENEAS;
							att = reader.getAttribute(S.scale);
							params.keysScale = (att!= null) ? Integer.parseInt(att) : 1;
							att = reader.getAttribute(S.translation);
							params.keysTranslation = (att!= null) ? Integer.parseInt(att) : 0;
						}

					else if(nodeName.equals(S.Introductor))
						params.introductor = (Introductor.Integer<?>)context.convertAnother(null, Class.forName(reader.getAttribute(S.clazz)));
			...
			try {
				while(reader.hasMoreChildren()){
					...
					if(nodeName.equals(S.Emisor)){
						Class<?> clazz = Class.forName(reader.getAttribute(S.clazz));
						Emisor emisor;
						if(reader.hasMoreChildren()){
						...
						if( (att = reader.getAttribute(S.cache)) != null )
							particulas.setEmisor(new Emisor.Cache(emisor,Integer.parseInt(att)));
						else
							particulas.setEmisor(emisor);
					}else if(nodeName.equals(S.Emision)){
						particulas.setEmision(Particulas.Emision.valueOf((String)context.convertAnother(null, String.class)));
						...
						if( (att = reader.getAttribute(S.p)) != null){
							...
							boolean s = Boolean.valueOf(reader.getAttribute(S.s));
	...
	public static void registerConverters(XStream xStream) {
		...
		xStream.alias(S.Getter_Integer, GetterIntegerConverter.IntegerParams.class);
		xStream.registerConverter(new GetterIntegerConverter());
	...
	public static void main(String... args){
		...
		XStreamConventers.GetterDoubleConverter.DoubleParams params = new XStreamConventers.GetterDoubleConverter.DoubleParams();
