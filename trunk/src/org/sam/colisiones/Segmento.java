...
public class Segmento {
	...
	private static int posicionRelaivaAlSegmento(double X1, double Y1,
			double X2, double Y2,
			double PX, double PY) {
		...
		double prs = Y2 * PX - X2 * PY;
		return (prs < 0.0) ? 1 : ((prs > 0.0) ? 2 : 0);
	}

	/**
	 * Coprueba si el segmento  (X1,Y1)(X2,Y2) corta al segmento (X1,Y1)(X2,Y2) 
	 *
	...
	public static boolean hayInterseccion(double X1, double Y1,
			double X2, double Y2,
			double X3, double Y3,
			double X4, double Y4) {
	...
	public boolean hayInterseccion(double X3, double Y3, double X4, double Y4){
