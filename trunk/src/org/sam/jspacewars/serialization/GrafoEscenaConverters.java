...
import org.sam.jogl.AtributosPoligono;
import org.sam.jogl.AtributosPunto;
	...
	private static class PoligonoConverter implements Converter {
		...
		public Object unmarshal(HierarchicalStreamReader reader, UnmarshallingContext context) {
			...
			while( i < nLados && reader.hasMoreChildren() ){
				reader.moveDown();
				coordX[i] = Float.parseFloat(reader.getAttribute(S.x))*scale + offX;
				coordY[i] = Float.parseFloat(reader.getAttribute(S.y))*scale + offY;
				reader.moveUp();
				i++;
			}
