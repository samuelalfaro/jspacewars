...
class ParticulasConShaders extends Particulas{
	...
	@Override
	public ParticulasConShaders clone() {
...
//	}
	...
	public void setEscala(InterpoladorDeValores iEscalas){
		ShaderAttributeSet shaderAttributeSet = ((ShaderAppearance)this.getAppearance()).getShaderAttributeSet();
	...
	private static Tuple2f[] escalaToArray( InterpoladorDeValores iEscalas){
		Tuple2f[] esclas = new Tuple2f[TAM];
		
		for(int i= 0; i< TAM; i++){
			float val = (float)iEscalas.getValue(((double)i)/(TAM-1));
			esclas[i] = new Point2f(val, val);
		}
		return esclas;
	}
	
	public void setEscala(InterpoladorDeValores iEscalaX, InterpoladorDeValores iEscalaY){
		ShaderAttributeSet shaderAttributeSet = ((ShaderAppearance)this.getAppearance()).getShaderAttributeSet();
		((ShaderAttributeValue)shaderAttributeSet.get(shaderAttrNames[1])).setValue(TRUE);
		((ShaderAttributeArray)shaderAttributeSet.get(shaderAttrNames[2])).setValue(escalaToArray(iEscalaX,iEscalaY));
	}
	private static Tuple2f[] escalaToArray( InterpoladorDeValores iEscalaX, InterpoladorDeValores iEscalaY){
		Tuple2f[] esclas = new Tuple2f[TAM];
		for(int i= 0; i< TAM; i++){
			double key = ((double)i)/(TAM-1);
			esclas[i] = new Point2f((float)iEscalaX.getValue(key), (float)iEscalaY.getValue(key));
		}
		return esclas;
	}
	...
	public void setColor( Color3f color, InterpoladorDeValores iAlfa){
		setColor(color.x, color.y, color.z, iAlfa);
	}
	public void setColor( float r, float g, float b, InterpoladorDeValores iAlfa){
		ShaderAttributeSet shaderAttributeSet = ((ShaderAppearance)this.getAppearance()).getShaderAttributeSet();
	...
	private static Color4f[] colorToArray( float r, float g, float b, InterpoladorDeValores iAlfa){
		Color4f[] colores = new Color4f[TAM];
		for(int i= 0; i< TAM; i++){
			double key = ((double)i)/(TAM-1);
			double  transp = iAlfa.getValue(key);
			colores[i] = new Color4f(r,g,b,(float)transp);
		}
		return colores;
	}

	public void setColor( InterpoladorPolinomico3D iColores, float alfa){
		ShaderAttributeSet shaderAttributeSet = ((ShaderAppearance)this.getAppearance()).getShaderAttributeSet();
		((ShaderAttributeValue)shaderAttributeSet.get(shaderAttrNames[3])).setValue(TRUE);
		((ShaderAttributeArray)shaderAttributeSet.get(shaderAttrNames[4])).setValue(colorToArray(iColores,alfa));
	}
	private static Color4f[] colorToArray( InterpoladorPolinomico3D iColores, float alfa){
		Color4f[] colores = new Color4f[TAM];
		for(int i= 0; i< TAM; i++){
			double key = ((double)i)/(TAM-1);
			double[] color = iColores.getObject(key);
			colores[i] = new Color4f((float)color[0],(float)color[1],(float)color[2],alfa);
		}
		return colores;
	}

	public void setColor( InterpoladorPolinomico3D iColores, InterpoladorDeValores iAlfa){
		ShaderAttributeSet shaderAttributeSet = ((ShaderAppearance)this.getAppearance()).getShaderAttributeSet();
		((ShaderAttributeValue)shaderAttributeSet.get(shaderAttrNames[3])).setValue(TRUE);
		((ShaderAttributeArray)shaderAttributeSet.get(shaderAttrNames[4])).setValue(colorToArray(iColores,iAlfa));
	}
	private static Color4f[] colorToArray( InterpoladorPolinomico3D iColores, InterpoladorDeValores iAlfa){
		Color4f[] colores = new Color4f[TAM];
		for(int i= 0; i< TAM; i++){
			double key = ((double)i)/(TAM-1);
			double[] color = iColores.getObject(key);
			double  transp = iAlfa.getValue(key);
			colores[i] = new Color4f((float)color[0],(float)color[1],(float)color[2],(float)transp);
		}
		return colores;
	}
