...
//					return (int)this.nval;
	...
	Geometria generarGeometria(){
		...
		//System.err.println("Numero de Vertices:"+nVertex);
		...
		//System.err.println("Tama�o de buffer Vertices:"+floatBuffer.capacity());
		...
		if(textCoord){
			...
			//System.err.println("Tama�o de buffer Coordenadas Textura:"+floatBuffer.capacity());
		...
		if(normal){
			floatBuffer = ByteBuffer.allocateDirect(N_BYTES_FLOAT * nVertex * 3).order(order).asFloatBuffer();
			//System.err.println("Tama�o de buffer Coordenadas Normales:"+floatBuffer.capacity());
	...
	OglList almacenarGeometria(GL gl){
		...
		gl.glBegin((flags & ObjLoader.TRIANGULATE)!=0 ?
				GL.GL_TRIANGLES:
				GL.GL_QUADS);
		while(!coordIdxList.isEmpty()){
			if(textCoord){
	...
	public static Objeto3D load(String filename, int flags) throws FileNotFoundException, ParsingErrorException{
		return load(new FileReader(filename),flags, null);
	}
	...
	public static Objeto3D load(URL url, int flags) throws IOException, ParsingErrorException{
		return load(new InputStreamReader(url.openStream()),flags, null);
	}
	...
	public static Objeto3D loadToList(String filename, int flags) throws FileNotFoundException, ParsingErrorException{
		return loadToList(new FileReader(filename),flags, null);
	}
	...
	public static Objeto3D loadToList(URL url, int flags) throws IOException, ParsingErrorException{
		return loadToList(new InputStreamReader(url.openStream()),flags, null);
	}
