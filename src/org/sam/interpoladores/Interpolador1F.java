...
final class Interpolador1F implements Getter.Float<Float> {
	...
	Interpolador1F(float keys[], Float[] values, MetodoDeInterpolacion mdi, Object... params) {
		this.keys = keys;
		this.funciones = Funcion.toFloat( mdi.generarFunciones( new ArrayExtractor.E1F(keys), new ArrayExtractor.Numerico<Float>(values), params )[0] );
	...
	Interpolador1F(int genKey, float scale, float translation, Float[] values, MetodoDeInterpolacion mdi, Object... params) {
		...
		this.funciones = Funcion.toFloat( funciones[0]);
		this.valorInicial = values[0];
		this.valorFinal = values[values.length-1];
	}
	...
	Interpolador1F(float keys[], float[] values, MetodoDeInterpolacion mdi, Object... params) {
		this.keys = keys;
		this.funciones = Funcion.toFloat( mdi.generarFunciones( new ArrayExtractor.E1F(keys), new ArrayExtractor.E1F(values), params )[0] );
		this.valorInicial = values[0];
		this.valorFinal = values[values.length-1];
	}
